...
import { ToastifyMessage, H4, Card, Button, PrivateGrid } from 'components';
...
export class Talents extends React.Component {
  ...
  setTalentsDetails = response => {
    if (get(response, 'status')) {
      ...
      this.setState({ projectList: array, cardList: cardArray, paginationData: response.data, isListLoading: false });
    } else {
      ...
      <React.Fragment>
        ...
        <PrivateGrid>
          <Content>
            ...
            <div className="d-flex flex-column justify-content-between flex-lg-row mb-4">
              ...
              <div className="d-flex">
                ...
                >
                  ...
                  <DropdownMenu right className="input-sm rc-slider-container">
            ...
            {this.state.cardView ? (
              this.talentListCard(projectList, isListLoading)
            ) : (
              <Card className="p-0 mb-0">{this.talentListTable(projectList, isListLoading)}</Card>
            )}
  ...
  talentListTable(projectList, isListLoading) {
    return (
      <DataTable
        noHeader
        columns={columns}
