...
export function hideSidebarFromURL() {
  const { pathname } = history.location;
  let isMatched = false;
  if (pathname === '/talent/my-profile') {
    isMatched = true;
  } else if (pathname.includes('/client/talent-profile/')) {
    isMatched = true;
  } else if (pathname.includes('/talent/talent-profile/')) {
    isMatched = true;
  } else {
    isMatched = false;
  }
